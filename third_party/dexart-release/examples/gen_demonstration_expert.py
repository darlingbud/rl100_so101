...
def parse_args():
    ...
    return args
def compute_return(reward: np.ndarray, not_done: np.ndarray, gamma: float == 0.99
    ) -> np.ndarray:
        size_ = len(reward)
...
def main():
    ...
    with tqdm(total=num_episodes) as pbar:
        ...
        while num_success < num_episodes:
            ...
            for j in range(env.horizon):
                ...
                if isinstance(next_obs, dict):
                    for key, value in next_obs.items():
                        next_obs[key] = value[np.newaxis, :]
                else:
                    next_obs = next_obs[np.newaxis, :]

                next_obs_image = next_obs['instance_1-rgb'][0]
                next_obs_image = (next_obs_image*255).astype(np.uint8)
                if next_obs_image.shape[0] != args.img_size:
                    next_obs_image = F.interpolate(torch.from_numpy(next_obs_image).permute(2,0,1).unsqueeze(0), 
                                            size=args.img_size).squeeze().permute(1,2,0).numpy()
                next_img_arrays_sub.append(next_obs_image)

                next_obs_point_cloud = next_obs['instance_1-point_cloud'][0] 
                if next_obs_point_cloud.shape[0] > args.num_points:
                    next_obs_point_cloud = downsample_with_fps(next_obs_point_cloud, num_points=args.num_points)
                next_point_cloud_arrays_sub.append(next_obs_point_cloud)

                next_depth_arrays_sub.append(next_obs['instance_1-depth'][0])
                next_state_arrays_sub.append(next_obs['state'][0])
                next_imagin_robot_arrays_sub.append(next_obs['imagination_robot'][0])
                reward_arrays_sub.append(reward)
                done_arrays_sub.append(done)

                obs = next_obs
                if j == env.horizon - 1 and not done:
                ...
                if done:
                    break
            # get last action from last obs
            action = policy.predict(observation=obs, deterministic=True)[0]
            next_action_arrays_sub = deepcopy(action_arrays_sub)
            next_action_arrays_sub.append(action)
            ...
            if eval_success:
                ...
                next_img_arrays.extend(next_img_arrays_sub)
                next_imagin_robot_arrays.extend(next_imagin_robot_arrays_sub)
                next_point_cloud_arrays.extend(next_point_cloud_arrays_sub)
                next_depth_arrays.extend(next_depth_arrays_sub)
                next_state_arrays.extend(next_state_arrays_sub)
                next_action_arrays.extend(next_action_arrays_sub)
                
                reward_arrays.extend(reward_arrays_sub)
                done_arrays.extend(done_arrays_sub)
                timeout_arrays.extend(timeout_arrays_sub)
    ...
    if next_img_arrays.shape[1] == 3: # make channel last
        next_img_arrays = np.transpose(next_img_arrays, (0,2,3,1))
    next_state_arrays = np.stack(next_state_arrays, axis=0)
    next_imagin_robot_arrays = np.stack(next_imagin_robot_arrays, axis=0)
    ...
    return_arrays = compute_return(reward_arrays, not_done_arrays, 0.99) # gamma=0.99
    ...
    # import pdb; pdb.set_trace()
    next_img_chunk_size = (env.horizon, next_img_arrays.shape[1], next_img_arrays.shape[2], next_img_arrays.shape[3])
    next_imagin_robot_chunk_size = (env.horizon, next_imagin_robot_arrays.shape[1], next_imagin_robot_arrays.shape[2])
    next_point_cloud_chunk_size = (env.horizon, next_point_cloud_arrays.shape[1], next_point_cloud_arrays.shape[2])
    next_depth_chunk_size = (env.horizon, next_depth_arrays.shape[1], next_depth_arrays.shape[2])
    next_state_chunk_size = (env.horizon, next_state_arrays.shape[1])
    next_action_chunk_size = (env.horizon, next_action_arrays.shape[1])
    reward_chunk_size = (env.horizon, 1)
    done_chunk_size = (env.horizon, 1)
    return_chunk_size = (env.horizon, 1)
    ...
    # import pdb; pdb.set_trace()
    zarr_data.create_dataset('next_img', data=next_img_arrays, chunks=next_img_chunk_size, dtype='uint8', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('next_state', data=next_state_arrays, chunks=next_state_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('next_imagin_robot', data=next_imagin_robot_arrays, chunks=next_imagin_robot_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('next_point_cloud', data=next_point_cloud_arrays, chunks=next_point_cloud_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('next_depth', data=next_depth_arrays, chunks=next_depth_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('next_action', data=next_action_arrays, chunks=next_action_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    ...
    zarr_data.create_dataset('timeout', data=timeout_arrays, chunks=done_chunk_size, dtype='bool', overwrite=True, compressor=compressor)
    ...
    cprint(f'next_imagin_robot shape: {next_imagin_robot_arrays.shape}, range: [{np.min(next_imagin_robot_arrays)}, {np.max(next_imagin_robot_arrays)}]', 'green')
