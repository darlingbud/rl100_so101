...
# Two-stage launcher for 3D flow policy:
# 1) Run one DDP stage-1 pass to materialize BC / IQL / dynamics artifacts.
# 2) Fan out single-GPU BPPO sweep jobs while sharing stage-1 artifacts and global best.
#
# Example:
#   bash scripts/train_policy_flow_two_stage.sh rl100 adroit_door_medium 0112 100 4
...
GPU_LIST=${GPU_LIST:-""}
LR_VALUES=${LR_VALUES:-"2.83e-6 1.42e-5 5.66e-6 3e-6 8.66e-6"}
...
root_run_dir="data/outputs_flow_two_stage/${exp_name}_seed${seed}/${act}/${encoder_type}/${model}"
...
run_stage1() {
    ...
    torchrun --standalone --nproc_per_node="${stage1_gpu_count}" train_ddp.py \
...
if [ "${RUN_SWEEP}" = true ] || [ "${RUN_SWEEP}" = "True" ]; then
    ...
    for combo in "${param_combinations[@]}"; do
        ...
        if [ "${batch_slot}" -ge "${num_available_gpus}" ]; then
            wait_for_batch
            batch_slot=0
        fi
    done
