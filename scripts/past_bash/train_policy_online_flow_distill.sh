# Mode 1: BC -> offline distill (after_dp) -> promote -> online RL with 1-step student
# Pipeline: 10-step flow BC -> distill to 1-step student -> promote student to self.model -> online PPO with 1-step policy
# Examples:
# bash scripts/train_policy_online_flow_distill.sh rl100 adroit_door_medium 0112 100
# bash scripts/train_policy_online_flow_distill.sh rl100 adroit_door_medium 0112 100
...
encoder_type='dp3vib'
model='skipnet'
run_dir="data/outputs/${exp_name}_seed${seed}/${act}/${encoder_type}/${model}"
