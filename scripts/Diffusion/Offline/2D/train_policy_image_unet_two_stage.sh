...
#
# Example:
#   bash scripts/train_policy_image_unet_two_stage.sh rl100 adroit_door_medium 0112 100 4
...
# Sweep grid
LR_VALUES=${LR_VALUES:-"2.83e-6 1.42e-5 5.66e-6 3e-5 8.66e-6"}
...
root_run_dir="data/outputs_2d/${exp_name}_seed${seed}/${act}/${encoder_type}/${model}"
...
run_stage1() {
    local params
    params=$(get_common_params "${stage1_run_dir}" "${STAGE1_BPPO_LR}" "${STAGE1_ROLLOUT_LENGTH}")
...
run_sweep_job() {
    ...
    sweep_run_dir="${sweep_root_dir}/${timestamp}-lr_${lr}_rollout_${rollout_length}"
    mkdir -p "${sweep_run_dir}"
    params=$(get_common_params "${sweep_run_dir}" "${lr}" "${rollout_length}")

    echo "[GPU ${gpu}] Starting sweep job: lr=${lr}, rollout=${rollout_length}"
    (
...
if [ "${RUN_SWEEP}" = true ] || [ "${RUN_SWEEP}" = "True" ]; then
    ...
    for lr in ${LR_VALUES}; do
        for rollout_length in ${ROLLOUT_VALUES}; do
            param_combinations+=("${lr}:${rollout_length}")
        done
    done
    ...
    for combo in "${param_combinations[@]}"; do
        IFS=':' read -r lr rollout_length <<< "${combo}"
        gpu=${GPU_ARRAY[$batch_slot]}

        run_sweep_job "${combo_idx}" "${gpu}" "${lr}" "${rollout_length}" &
        batch_pids+=($!)
