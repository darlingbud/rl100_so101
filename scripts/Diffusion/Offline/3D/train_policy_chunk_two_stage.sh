...
config_name=${config_name:-'rl100_3d_epsilon'}
...
# HORIZON must satisfy horizon - (n_obs_steps - 1) == n_action_steps under
# no_pre_action=True (the rl100_3d_epsilon default), otherwise the policy
# predicts a short chunk while the critic/IQL/dynamics slice [n_obs_steps-1 :
# n_obs_steps-1 + n_action_steps] and hard-error. Keep this consistent with the
# other (2D / online / flow) chunk launchers.
...
get_common_params() {
    ...
    local offline_chunk_ratio_mode=${7:-per_step}
