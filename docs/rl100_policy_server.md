...
```text
/home/tianma/work/RL-100/RL-100/data/outputs/ro101_clip/42/resume_best465_fixed_kl_400ep/checkpoints/latest.ckpt
```

启动时同时使用该次训练保存的完整解析配置：

```text
/home/tianma/work/RL-100/RL-100/data/outputs/ro101_clip/42/resume_best465_fixed_kl_400ep/config.yaml
```
...
RUN_DIR=/home/tianma/work/RL-100/RL-100/data/outputs/ro101_clip/42/resume_best465_fixed_kl_400ep
...
该 latest checkpoint 对应 epoch 865、global step 50,170，且没有 EMA state，
...
./scripts/run_lerobot_policy_client.sh \
  ...
  --inference-fps 3 \
  --duration 10 \
  --action-log outputs/latest_epoch865_dry_run.jsonl
```
...
./scripts/run_lerobot_policy_client.sh \
  ...
  --execute \
  --action-log outputs/latest_epoch865_execute.jsonl
```

执行命令不会跳过确认，仍需在终端输入 `MOVE`。需要跳过重新标定时增加
`--no-calibrate`；只有已经确认当前标定文件与机器人匹配时才能使用。
当前 checkpoint 的本机 smoke 推理耗时约 269 ms，因此先使用 3 Hz 推理；4 步
...
当前真实 epoch 865 `latest.ckpt` 和 RO101 数据完成完整请求。
