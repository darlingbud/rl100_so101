    ...
    def copy_from_store(cls, src_store, store=None, keys=None, 
        ...
        for key, value in buffer.items():
            ...
            try:
                ...
                # 不是数值数组，可能是 bytes array
                # 这里我们只打印 bytes 长度区间，避免把整个 bytes dump 出来
                try:
                    flat = list(value.flat)
                    # 只对第一个元素判定类型：bytes 或 bytearray
                    if isinstance(flat[0], (bytes, bytearray)):
                        lengths = [len(x) for x in flat]
                        range_str = f"bytes len [{min(lengths)}~{max(lengths)}]"
                    else:
                        # 其它 object 类型，直接 str() 一把抓
                        range_str = str(value)
                except Exception:
                    # 万一连 .flat 也不行
                    range_str = repr(value)

            cprint(
                f"Replay Buffer: {key}, "
                f"shape={shape}, dtype={dtype}, content={range_str}",
                "green"
            )
        cprint("--------------------------", "green")
        return buffer
