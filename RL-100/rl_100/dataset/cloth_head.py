...
class ClothHead(BaseDataset):
    def __init__(self,
        ...
        self.img_shape = {'rgb_head': rgb_head_shape, 'next_rgb_head': rgb_head_shape}
        self.use_velocity = use_velocity
        
        # Use fast loading with only required keys
        required_keys = ['state', 'action', 'rgb_head', 'next_rgb_head', 'next_state', 'next_action', 'reward', 'done', 'timeout', 'return']
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        # action = deepcopy(self.replay_buffer['action'])
        # left_action = transform_to_9d_batch(action[:,:7],action[:,[7]])
        # right_action = transform_to_9d_batch(action[:,8:15],action[:,[15]])
        # action = np.concatenate([left_action, right_action], axis=-1)
        # next_action = deepcopy(self.replay_buffer['next_action'])
        # next_left_action = transform_to_9d_batch(next_action[:,:7],next_action[:,[7]])
        # next_right_action = transform_to_9d_batch(next_action[:,8:15],next_action[:,[15]])
        # next_action = np.concatenate([next_left_action, next_right_action], axis=-1)
    ...
    def _sample_to_data(self, sample):
        ...
        rgb_head = sample['rgb_head'][:,].astype(np.float32) # (T, 3, 64, 64) # TODO need to convert to BGR from bytes
        next_rgb_head = sample['next_rgb_head'][:,].astype(np.float32) # (T, 3, 64, 64) # TODO need to convert to BGR from bytes
        ...
        data = {
            'obs': {
                'rgb_head': rgb_head, # T, 3, 64, 64
                'agent_pos': agent_pos, # T, D_pos
            },
            'next_obs': {
                'rgb_head': next_rgb_head, # T, 3, 64, 64
