...
class MetaworldDataset(BaseDataset):
    def __init__(self,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'point_cloud', 'img', 'next_state', 'next_action', 'next_point_cloud', 'next_img', 'reward', 'done', 'full_state', 'return'])
    ...
    def get_shape_info(self, n_action_steps, n_obs_steps):
        ...
        pc_shape = list(pc_shape)  # Convert to list
        pc_shape[1] = 512          # Modify the element
        pc_shape = tuple(pc_shape)
        shape_info = {
        'obs': {
            'point_cloud': (n_obs_steps,) + pc_shape[1:],
            'agent_pos': (n_obs_steps,) + agent_pos.shape[1:],
            'image': (n_obs_steps,) + (3, 84, 84),
        },
