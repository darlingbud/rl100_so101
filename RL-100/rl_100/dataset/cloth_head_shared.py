...
import os
import copy
from rl_100.dataset.cloth_head import ClothHead
...
class ClothHeadShared(ClothHead):
    """
    ClothHead dataset that uses shared memory to avoid duplicating data across DDP processes.
    ...
    Usage:
        ...
        2. In each DDP process:
           dataset = ClothHeadShared(zarr_path=zarr_path, shared_memory_info_path=info_path, ...)
    """
    ...
    def __init__(self,
        ...
        if shared_memory_info_path and os.path.exists(shared_memory_info_path):
            try:
                ...
                # Filter only the keys we need for ClothHead
                required_keys = ['state', 'action', 'rgb_head', 'next_rgb_head', 'next_state', 'next_action', 'reward', 'done', 'timeout', 'return']
                filtered_data = {
                    'meta': shared_data.get('meta', {}),
                    'data': {k: v for k, v in shared_data['data'].items() if k in required_keys}
                }
                data = filtered_data
    ...
    def _sample_to_data(self, sample):
        ...
        next_rgb_head = sample['next_rgb_head'][:,].astype(np.float32) # (T, H, W, C) - keep original format
        ...
        # rgb_head and next_rgb_head stay as (T, H, W, C)
        ...
        data = {
            ...
            'next_obs': {
                'rgb_head': next_rgb_head, # T, H, W, C (320, 240, 3)
