...
def compute_return(reward, not_done, gamma: float == 0.99
        ...
        return return_
class d4rl_dataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'next_state', 'next_action', 'reward', 'done', 'return'])
    ...
    def get_shape_info(self, ):
        ...
        shape_info = {
        'obs': {
            'agent_pos': agent_pos.shape[:],
        },
        'action': sample['action'].shape[:],
        }
        return shape_info
