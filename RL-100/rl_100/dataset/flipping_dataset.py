...
def compute_return(reward, not_done, gamma: float == 0.99
        ...
        return return_
class FlippingDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'point_cloud', 'next_action', 'reward', 'done', 'timeout', 'return'])
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        # 使用索引偏移生成next数据，保持与_sample_to_data的一致性
        state_data = self.replay_buffer['state'][...,:] 
        point_cloud_data = self.replay_buffer['point_cloud']
        
        # 生成next数据: next = data[1:] + 最后一帧补齐
        next_agent_pos = np.concatenate([state_data[1:], state_data[-1:]], axis=0)
        next_point_cloud = np.concatenate([point_cloud_data[1:], point_cloud_data[-1:]], axis=0)
    ...
    def _sample_to_data(self, sample):
        ...
        # 使用索引偏移生成next_obs，节省内存
        # next_state = state[1:, ...] 再补齐最后一位 
        next_agent_pos = np.concatenate([agent_pos[1:], agent_pos[-1:]], axis=0).astype(np.float32)
        next_point_cloud = np.concatenate([point_cloud[1:], point_cloud[-1:]], axis=0).astype(np.float32)
