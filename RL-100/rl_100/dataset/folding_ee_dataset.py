...
class FoldingDataset(BaseDataset):
    def __init__(self,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'ee_pose', 'point_cloud', 'next_state', 'next_action', 'next_ee_pose', 'next_point_cloud', 'reward', 'done', 'timeout', 'return'])
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        data = {
            ...
            'agent_pos': self.replay_buffer['ee_pose'][...,:],
            'point_cloud': self.replay_buffer['point_cloud'],

            'next_action': self.replay_buffer['next_action'],
            'next_agent_pos': self.replay_buffer['next_ee_pose'][...,:],
    ...
    def _sample_to_data(self, sample):
        agent_pos = sample['ee_pose'][:,].astype(np.float32) # (agent_posx2, block_posex3)
        ...
        next_agent_pos = sample['next_ee_pose'][:,].astype(np.float32) # (agent_posx2, block_posex3)
