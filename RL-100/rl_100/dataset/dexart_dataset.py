...
class DexArtDataset(BaseDataset):
    def __init__(self,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['state', 'action', 'point_cloud', 'img', 'imagin_robot', 'next_state', 'next_action', 'next_point_cloud', 'next_img', 'next_imagin_robot', 'reward', 'done', 'timeout', 'return'])
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        normalizer['next_imagin_robot'] = SingleFieldLinearNormalizer.create_identity()
        normalizer['next_point_cloud'] = SingleFieldLinearNormalizer.create_identity()
        return normalizer
    ...
    def _sample_to_data(self, sample):
        ...
        next_imagin_robot = sample['next_imagin_robot'][:,].astype(np.float32)
        next_image = sample['next_img'][:,].astype(np.float32)
        data = {
            'obs': {
            ...
            'next_obs': {
                'point_cloud': next_point_cloud, # T, 512, 3
    ...
    def get_shape_info(self, n_action_steps, n_obs_steps ):
        ...
        'obs': {
            ...
            'imagin_robot': (n_obs_steps,) + imagin_robot.shape[1:],
        },
