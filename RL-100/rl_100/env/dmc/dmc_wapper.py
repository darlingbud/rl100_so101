...
from rl_100.gym_util.mjpc_wrapper import point_cloud_sampling
...
class DMCEnv(Env):
    def __init__(
        ...
        observation_spec = deepcopy(self.env.observation_spec())
        if 'pixels' in observation_spec:
            del observation_spec['pixels']
        sensor_dim = 0
        for key, spec in observation_spec.items():
            if len(spec.shape) == 0:
                continue    
            sensor_dim += spec.shape[0]
            print(f"{key} dimension:", spec.shape)
    ...
    def get_obs(self, time_step=None):
        ...
        obs = {}
        info = {}
        state_only = deepcopy(time_step.observation)
        if 'pixels' in state_only:
            del state_only['pixels']
        obs['state'] = _flatten_obs(state_only)
        if self.obs_mode == "state":
            ...
            # img and depth
            obs["image"] = deepcopy(rgb)
            depth[~sign] = 0  # Filter pixels are too far away
            depth = depth / self.max_depth
            obs["depth"] = deepcopy(np.float16(depth)[..., None])
            ...
            if self.obs_mode in ["xyz", "pointcloud", "xyz-img", "xyz-rgb-img"]:
                ...
                if self.obs_mode in ["xyz-rgb-img", "pointcloud"]:
                    obs["rgb"] = rgb
                obs['xyz-rgb'] = np.concatenate([xyz, rgb], axis=-1)
    ...
    def step(self, action):
        ...
        obs_dict = {
            ...
            'depth': obs['depth'],
            'agent_pos': obs['state'],
            'point_cloud': obs['xyz-rgb'],
        }
        return obs_dict, time_step.reward, time_step.last(), {'discount': time_step.discount}
    def step_(self, action):
        ...
        return obs_dict, reward, done, {'discount': time_step.discount}
