...
LOAD_FROM_DATA = True
dir_name = 'data_20250312_150_static_20_200'
from rtde_control import RTDEControlInterface
rtde_c = RTDEControlInterface('192.169.0.10')
        ...
        for file_name in file_names:
            ...
            for i in range(len(demo)):
                ...
                point_cloud = depth2pc(
                    ...
                    (231.529296875, 231.78125, 146.9921875, 128.482421875),
                    X_root_camera
                )
                # print(time.time() - t)
                if i > 0:
                    reward, _ = calc_reward(frame['tag_poses'], prev_tag_poses = pre_tag_poses[0], static_penalty=0.4, current_action=frame['action'], prev_action=pre_action)
                else:
                    reward, _ = calc_reward(frame['tag_poses'], static_penalty=0.0)
                pre_tag_poses.append(frame['tag_poses'])
                pre_action = frame['action']
                # reward, _ = calc_reward(frame['tag_poses'])
                # print("reward:", reward)
                if reward == 20:
                    assert i == len(demo) - 1, f"{i}, {len(demo) - 1}"
                ...
                if USE_XY_AGENT_POS:
                    data['agent_pos'].append(rtde_c.getForwardKinematics(frame['qpos'])[:2])
                else:
            ...
            if SHOW_REWARD_CURVE:
                ...
                plt.grid()
                plt.show()
        np.save(PUSHT_DEMO_DIR / f"{dir_name}.npy", data)

    else:   
        demo_path = PUSHT_DEMO_DIR / f"{dir_name}.npy"
        demo = np.load(demo_path, allow_pickle=True)
        data = demo.item()
        traj_end_index = np.where(data['timeout'])[0] # get the index of the last step of each episode
        save_dir = PUSHT_DATA_DIR / f"{dir_name}.zarr"
