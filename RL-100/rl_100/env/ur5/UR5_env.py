...
class UR5Env:
    ...
    def __init__(
        ...
        self.pre_tag_poses = deque(maxlen=60)
    ...
    def step(self, action):
        ...
        if not self.done:
            ...
            while True:
                bound = 1.15
                x, y = self._cur_tcp[0], self._cur_tcp[1]
                if x - y >= bound:
                    delta = (x - y) - bound
                    self._cur_tcp[0] -= delta / np.sqrt(2)
                    self._cur_tcp[1] += delta / np.sqrt(2)
                if self._cur_tcp[0] - self._cur_tcp[1] < bound:
                    break
        ...
        # tag_poses = self.camera.detect_apriltag(frame['color'])
        if self.env_step > 1:
            reward, is_success = calc_reward(tag_poses, prev_tag_poses = self.pre_tag_poses[0], static_penalty=0., prev_action=self.pre_action, current_action=action)
        else:
            reward, is_success = calc_reward(tag_poses, static_penalty=0.0, prev_action=None, current_action=None)
