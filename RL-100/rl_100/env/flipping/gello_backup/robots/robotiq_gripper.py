...
class RobotiqGripper:
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        # 对于 Robotiq 140 夹爪，完全闭合时手指会接触，检测为 STOPPED_INNER_OBJECT
        # 这种情况下使用当前位置作为最大闭合位置
        if RobotiqGripper.ObjectStatus(status) == RobotiqGripper.ObjectStatus.STOPPED_INNER_OBJECT:
            if log:
                print(f"Gripper fingers touched at position {position}, using as max position")
        elif RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            raise RuntimeError(
    ...
    def move(self, position: int, speed: int, force: int) -> Tuple[bool, int]:
        ...
        :param position: Position to move to [0, 255], will be mapped to calibrated range
        ...
        # 将输入 [0, 255] 线性映射到校准后的 [_min_position, _max_position]
        position = clip_val(0, position, 255)  # 先确保输入在 0-255 范围内
        mapped_pos = int(self._min_position + position * (self._max_position - self._min_position) / 255)
