    ...
    def serve(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                elif method == "command_cartesian_state":
                    result = self._robot.command_cartesian_state(**args)
...
class ZMQClientRobot(Robot):
    ...
    def command_cartesian_state(self, cartesian_state: np.ndarray) -> None:
        """Command the leader robot to the given cartesian state.

        Args:
            cartesian_state (np.ndarray): [x, y, z, rx, ry, rz, gripper, ...] 笛卡尔空间状态
        """
        request = {
            "method": "command_cartesian_state",
            "args": {"cartesian_state": cartesian_state},
        }
        send_message = pickle.dumps(request)
        self._socket.send(send_message)
        result = pickle.loads(self._socket.recv())
