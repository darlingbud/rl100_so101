...
class DDIMSchedulerExtended(DDIMScheduler):
  ...
  def step_logprob(
    ...
    std_dev_t = (eta * variance ** (0.5)).to(sample.device)
  ...
  def step_forward_logprob(
    ...
    if return_debug:
      return log_prob, {'mean': prev_sample, 'std': std_dev_t}
    return log_prob
  def step_forward_logprob_with_entropy(
    ...
    if return_debug:
      return log_prob, entropy, {'mean': prev_sample, 'std': std_dev_t}
