...
class PointNetImaginationExtractorGP(BaseFeaturesExtractor):
    def __init__(self, 
        ...
        if extractor_name == "smallpn":
            ...
            self.extractor = PointNet(point_channel=6)
        ...
        self.final_projection = nn.Linear(256, out_channel)
        if self.use_state:
            self.state_dim = self.state_space[0]
    ...
    def forward(self, observations: TensorDict) -> th.Tensor:
        ...
        points = observations[self.point_cloud_key]  # B * N * 3
        ...
        # points: B * 3 * (N + sum(Ni))
        pn_feat = self.final_projection(self.extractor(points) )   # B * 256
        if self.use_state:
