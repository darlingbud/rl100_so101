...
from rl_100.common.pytorch_util import replace_submodules
import einops
...
class TimmObsEncoder(ModuleAttrMixin):
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            if self.n_emb is not None:
                feature = self.key_projection_map[key](feature)
            assert len(feature.shape) == 2 and feature.shape[0] == B_t
        ...
        if self.use_agent_pos:
            for key in self.low_dim_keys:
                ...
                # assert B == batch_size
                if self.n_emb is not None:
                    data = self.key_projection_map[key](data)
    ...
    @torch.no_grad()
    def output_shape(self, n_obs_steps):
