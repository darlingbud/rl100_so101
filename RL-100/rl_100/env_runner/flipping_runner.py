...
from rl_100.gym_util.multistep_wrapper_real import MultiStepWrapper, aggregate, dict_take_last_n
...
from rl_100.env import FlippingEnv


class Flip(BaseRunner):
    def __init__(self,
                 output_dir,
                 eval_episodes=20,
                 max_steps=200,
        ...
        if self.fake_env:
            ...
            self.env = MultiStepWrapper(
                FlippingEnv(dt=dt, num_point_cloud=num_points, max_steps=max_steps),
    ...
    def run(self, policy: BasePolicy, data_collect = False, use_cm = False, distill2mean=False, traj_path = None): 
        ...
        if self. fake_env:
            ...
            while completed_episodes < self.eval_episodes:
                ...
                # worker = PolicyInferenceWorker(policy, deterministic=deterministic, use_cm=use_cm, distill2mean=distill2mean)
                # worker.policy.reset()
                # worker.start()
                ...
                # demo = np.load('./data/demo_000.npy', allow_pickle=True)
                # actions = []
                # for frame in demo:
                ...
                # 记录初始状态 s0（用于后续构建正确的 state_arrays）
                initial_state = obs['agent_pos'][-1].copy()
                initial_point_cloud = obs['point_cloud'][-1].copy()
                initial_depth = obs['depth'][-1].copy()
                initial_depth_scale = obs['depth_scale'][-1].copy()
                ...
                while not done:
                    ...
                    obs_dict = dict_apply(obs_dict_input, lambda x: torch.from_numpy(x).to(device=device))
                    obs_dict_input = {
                    ...
                    # import pdb; pdb.set_trace()
                    # worker.infer_async(obs_dict_input)
                    # action_dict = worker.get_action()
                    ...
                    pre_reward = reward
                    if np.array(info['is_success']).any() and not is_success:
                        is_success = True
                        hard_success += 1  # 确保 hard_success 已定义
                        
                    actual_step_count += 1
                    # while time.time() - time_action < self.dt:
                ...
                # 用 s0 + next[:-1] 构建正确的 state_arrays
                # state[i] 是执行 action[i] 前的状态，next_state[i] 是执行后的状态
                # 所以 state = [s0] + next_state[:-1]
                state_arrays = [initial_state] + list(next_state_arrays[:-1])
                point_cloud_arrays = [initial_point_cloud] + list(next_point_cloud_arrays[:-1])
                depth_arrays = [initial_depth] + list(next_depth_arrays[:-1])
                depth_scale_arrays = [initial_depth_scale] + list(next_depth_scale_arrays[:-1])
                # time.sleep(3)
                ...
                print("########################################################")
                print("point_cloud_arrays: ", np.array(point_cloud_arrays).shape)
                print("depth_arrays: ", np.array(depth_arrays).shape)
                print("depth_scale_arrays: ", np.array(depth_scale_arrays).shape)
                print("state_arrays: ", np.array(state_arrays).shape)
                print("action_arrays: ", np.array(action_arrays).shape)
                print("next_point_cloud_arrays: ", np.array(next_point_cloud_arrays).shape)
                print("next_depth_arrays: ", np.array(next_depth_arrays).shape)
                print("next_depth_scale_arrays: ", np.array(next_depth_scale_arrays).shape)
                print("next_state_arrays: ", np.array(next_state_arrays).shape)
                print("next_action_arrays: ", np.array(next_action_arrays).shape)
                print("reward_arrays: ", np.array(reward_arrays).shape)
                print("done_arrays: ", np.array(done_arrays).shape)
                print("timeout_arrays: ", np.array(timeout_arrays).shape)
                print("is_success_arrays: ", np.array(is_success_arrays).shape)
                print("########################################################")
                print("success rate: ", hard_success / (completed_episodes+1))
                print("is success: ", is_success)
                # exit()
                if traj_path is None:
                    traj_path = '/media/guoping/02/flipping_rollout_20260105/'
                # 确保目录存在
                ...
                pbar.update(1)
                if completed_episodes < self.eval_episodes:
                    obs = env.reset()
    ...
    def idql_run(self, policy: BasePolicy, dynamics, first_action, get_np, use_gae, iql, Q, repeat_num, traj_path = None, data_collect = None, use_cm=False, distill2mean=False):
        ...
        if self. fake_env:
            ...
            for episode_idx in tqdm.tqdm(range(self.eval_episodes),
                ...
                # import pdb; pdb.set_trace()
                # if data_collect:
                # 使用传入的 traj_path 参数，如果为 None 则使用默认路径
                if traj_path is None:
                    traj_path = '/media/guoping/02/flipping_rollout_20260105/'
                # 确保目录存在
                os.makedirs(traj_path, exist_ok=True)
