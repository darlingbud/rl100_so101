...
from rl_100.env import FrankaEnv
class Rotate(BaseRunner):
    def __init__(self,
                 output_dir,
                 eval_episodes=20,
                 max_steps=200,
        ...
        if self.fake_env:
            ...
            self.env = MultiStepWrapper(
                FrankaEnv(num_point_cloud=num_points, use_rgb=use_rgb),
    ...
    def run1(self, policy: BasePolicy, use_cm = False, distill2mean=False, traj_path=None):  
        if self. fake_env:
            ...
            _ = env.reset()  # Unnecessary reset at the end
            del env
        ...
        if self. fake_env:
            ...
            for episode_idx in tqdm.tqdm(range(self.eval_episodes),
                ...
                while not done:
                    ...
                    image_arrays.append(obs['image'][-1])
                    ...
                    next_image_arrays.append(obs['image'][-1])
                    ...
                    is_stage_1_arrays.append(info['is_stage_1'][-1])
                    is_fail_arrays.append(info['is_fail'][-1])
                ...
                with h5py.File(hdf5_filename, 'w') as hdf5_file:
                    ...
                    if self.save_image:
                        hdf5_file.create_dataset('image', data=np.array(image_arrays))
                        hdf5_file.create_dataset('next_image', data=np.array(next_image_arrays))
                    ...
                    hdf5_file.create_dataset('is_stage_1', data=np.array(is_stage_1_arrays))
                    hdf5_file.create_dataset('is_fail', data=np.array(is_fail_arrays))
            ...
            return log_data
    def data_collect(self, policy: BasePolicy, use_cm = False, distill2mean=False):  
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes),
            ...
            while not done:
                ...
                done_arrays.append(done)
                timeout_arrays.append(info['timeout'])
                if reward is None:
                    reward = pre_reward
                episode_reward += reward
                reward_arrays.append(reward)
            ...
            # 将本 episode 的数据保存到 HDF5 文件中
            hdf5_filename = f'eval_episode_{episode_idx}.h5'
            with h5py.File(hdf5_filename, 'w') as hdf5_file:
                hdf5_file.create_dataset('point_cloud', data=np.array(point_cloud_arrays))
                hdf5_file.create_dataset('state', data=np.array(state_arrays))
                hdf5_file.create_dataset('action', data=np.array(action_arrays))
        ...
        return log_data
    def idql_run(self, policy: BasePolicy, dynamics, first_action, get_np, use_gae, iql, Q, repeat_num, use_cm=False, distill2mean=False):
        if self. fake_env:
            all_goal_achieved = np.random.randint(90, 100)
            all_success_rates = np.random.randint(90, 100)
            all_returns = 100
